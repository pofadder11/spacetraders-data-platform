...
class ShipyardOfferSnapshot(Base):
    __tablename__ = "shipyard_offers"

    waypoint_symbol: Mapped[str] = mapped_column(String(100), primary_key=True, index=True)
    ship_type: Mapped[str] = mapped_column(String(100), primary_key=True, index=True)
    observed_at: Mapped[datetime] = mapped_column(DateTime(timezone=True), primary_key=True, index=True)
...
class ShipyardOfferCurrent(Base):
    __tablename__ = "shipyard_offers_current"
